...
const mockProgressData = {
  dateInfo: "Today, April 3",
  dailyConsumption: {
    calories: { current: 1850, target: 2200, unit: "kcal", color: "text-teal-500", bg: "bg-teal-500" },
    protein: { current: 140, target: 160, unit: "g", color: "text-emerald-500", bg: "bg-emerald-500" },
    carbs: { current: 150, target: 200, unit: "g", color: "text-blue-500", bg: "bg-blue-500" },
    fats: { current: 65, target: 84, unit: "g", color: "text-orange-500", bg: "bg-orange-500" }
  },
  weeklySummary: {
    weekRange: "Mar 28 - Apr 3",
...
const CircleProgress = ({ label, current, target, unit, color, bg }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <p className="text-xs font-bold text-slate-400">Target: {target}</p>
...
export default function Progress() {
  ...
  const { dateInfo, dailyConsumption, weeklySummary } = mockProgressData;
      ...
      <div className="animate-fade-in font-body pb-12">
        <div className="max-w-6xl mx-auto space-y-10">
          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-slate-200 pb-8 gap-6">
            ...
            <button className="flex items-center gap-3 bg-white/80 backdrop-blur-sm border border-slate-200 px-7 py-4 rounded-full shadow-sm hover:shadow-md hover:bg-white transition-all cursor-pointer text-slate-600 font-bold group hover:-translate-y-0.5">
              ...
              <span className="tracking-wide text-[15px]">{dateInfo}</span>
          ...
          {/* 2. Top Section: Today's Consumption Rings */}
          ...
          {/* 3. Bottom Section: Weekly Insights */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
            
            {/* Card A: Weekly Averages */}
            <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-teal-900/5 border border-white flex flex-col h-full hover:shadow-2xl hover:shadow-teal-900/10 transition-all duration-500">
              <div className="flex justify-between items-center mb-10 border-b border-slate-100 pb-6">
                ...
                <span className="text-[11px] font-extrabold text-slate-500 bg-slate-100 px-4 py-2 rounded-full tracking-widest uppercase">{weeklySummary.weekRange}</span>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-center space-y-10">
                <div className="flex items-center justify-between group">
                  <div>
                    ...
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{weeklySummary.averageCalories} <span className="text-lg font-bold text-slate-400 tracking-widest uppercase">kcal</span></span>
                  </div>
                ...
                <div className="flex items-center justify-between group">
                  <div>
                    <span className="block text-xs font-extrabold text-slate-400 uppercase tracking-widest mb-2 transition-colors group-hover:text-emerald-600">Net Deficit</span>
                    <span className="text-4xl font-black text-emerald-500 tracking-tight">{weeklySummary.netDeficit}</span>
                  </div>
                  <div className="w-14 h-14 rounded-[1.25rem] bg-emerald-50 text-emerald-500 border border-emerald-100 flex items-center justify-center shadow-inner transition-all duration-300 group-hover:scale-110">
                ...
                <div className="flex items-center justify-between group">
                  <div>
                    ...
                    <span className="text-4xl font-black text-teal-600 tracking-tight">{weeklySummary.consistencyScore}</span>
                  </div>
            ...
            <div className="bg-gradient-to-br from-teal-50 via-white to-teal-50/50 backdrop-blur-md rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-teal-900/5 border border-white flex flex-col h-full relative overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-teal-900/10">
              ...
              <div className="flex justify-between items-start mb-10 relative z-10">
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-bold">Volume over the last 7 days</p>
                </div>
                <span className="material-symbols-outlined text-teal-400 text-4xl mt-1 opacity-70">bar_chart_4_bars</span>
              </div>

              {/* CSS Stylized Bar Chart Placeholder */}
              <div className="flex-1 flex items-end justify-between gap-3 md:gap-4 mt-auto relative z-10 h-56 border-b-2 border-slate-100 pb-3">
                {[45, 70, 50, 85, 95, 60, 90].map((height, i) => (
                  <div key={i} className="w-full flex flex-col items-center gap-3 group h-full justify-end cursor-pointer">
                    {/* Tooltip hover effect */}
                    <span className="text-[11px] font-extrabold text-teal-700 bg-teal-50 px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:-translate-y-1 duration-300 border border-teal-100 shadow-sm">
                      {Math.floor(height * 23.5)}
                    </span>
                    <div 
                      className={`w-full max-w-[3rem] min-w-[1.5rem] rounded-t-[14px] transition-all duration-500 delay-75 group-hover:opacity-90 
                                ${i === 6 ? 'bg-gradient-to-t from-teal-500 to-teal-400 shadow-[0_-8px_20px_rgba(20,184,166,0.3)]' : 'bg-teal-200/40 group-hover:bg-teal-300/60'}`}
                      style={{ height: `${height}%` }}
                    ></div>
                  </div>
                ))}
              </div>
              <div className="flex justify-between px-2 md:px-3 mt-5 text-[11px] font-extrabold text-slate-400 relative z-10 uppercase tracking-widest w-full">
                <span>Mon</span>
                <span>Tue</span>
                <span>Wed</span>
                <span>Thu</span>
                <span>Fri</span>
                <span>Sat</span>
                <span className="text-teal-600 bg-teal-50 px-2 py-1 rounded-md -my-1 border border-teal-100">Sun</span>
              </div>
            </div>
